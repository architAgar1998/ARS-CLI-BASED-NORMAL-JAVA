...
class UserInfoDAOImplTest {
	...
	public void testUserRegister() {
		...
		boolean expected = userInfoDAO.userRegister(userBean);
	...
	public void testUserAuthentication() {
		...
		userBean.setPassword("12345");
		userBean.setRole("user");
		boolean expected = userInfoDAO.userAuthentication(userBean.getUserId(), userBean.getPassword(),
				userBean.getRole());
		assertEquals(expected, true);
	...
	public void testGetAllUsers() {
		Collection<UserInfoBean> allUsers = userInfoDAO.getAllUsers();
	...
	public void testUpdateUserDetails() {
		...
		userBean.setRole("user");
		try {
			boolean expected = userInfoDAO.updateUserDetails(userBean.getUserId(), userBean);
		} catch (Exception e) {
			assertThrows(UserDetailsException.class, () -> {
				userInfoDAO.updateUserDetails(userBean.getUserId(), userBean);
			});
		}
	}
